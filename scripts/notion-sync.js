...
async function setupDatabase() {
  ...
  const updates  = {};
  if (existing.includes('이름') && !existing.includes('날짜')) updates['이름'] = { name: '날짜' };
  if (existing.includes('Name') && !existing.includes('날짜')) updates['Name'] = { name: '날짜' };
  const numCols = [
    '공포탐욕지수','USD/KRW','EUR/USD','USD/JPY','GBP/USD',
    'NVDA 가격','NVDA 등락(%)','AAPL 가격','AAPL 등락(%)',
    'TSLA 가격','TSLA 등락(%)','MSFT 가격','MSFT 등락(%)',
    'GOOGL 가격','GOOGL 등락(%)','META 가격','META 등락(%)',
    'AMZN 가격','AMZN 등락(%)','AVGO 가격','AVGO 등락(%)',
    'PLTR 가격','PLTR 등락(%)',
  ];
  ...
  if (!existing.includes('공포탐욕 라벨')) {
    updates['공포탐욕 라벨'] = { select: { options: [
      { name: '극단적 공포', color: 'red' },{ name: '공포', color: 'orange' },
      { name: '중립', color: 'yellow' },{ name: '탐욕', color: 'green' },
...
async function fetchStocks() {
  ...
  await Promise.allSettled(symbols.map(async s => {
    try { out[s] = await fetchStock(s); } catch { out[s] = { price: null, changePct: null }; }
  }));
  return out;
}
...
async function syncToNotion(fng, rates, stocks) {
  ...
  const props = {
    ...
    'USD/KRW': { number: rates.usdKrw },'EUR/USD': { number: rates.eurUsd },
    'USD/JPY': { number: rates.usdJpy },'GBP/USD': { number: rates.gbpUsd },
  };
  for (const s of ['NVDA','AAPL','TSLA','MSFT','GOOGL','META','AMZN','AVGO','PLTR']) {
...
async function main() {
  ...
  console.log(`📊 공포탐욕지수(CNN): ${fng.value} (${FNG_KR[fng.label] || fng.label})`);
  console.log(`💱 USD/KRW: ${rates.usdKrw}`);
  console.log(`📈 NVDA: $${stocks.NVDA?.price ?? 'N/A'} | AAPL: $${stocks.AAPL?.price ?? 'N/A'}`);
